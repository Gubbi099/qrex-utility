...
for k,v in pairs(bdChannels) do
  ...
  if(channel == "") then
    ...
    MsgC(Color(255,255,255), "Found channel: ", Color(155,155,155), q.name .."\n")
    end
  end
end
...
addCommand("printtable", "Test command", function() PrintTable(vulChannel) end)
addCommand("help", "Shows you a list of commands and their description.", help)
addCommand("pl_intensescan", "Launches an intense scan on the server (This can be risky).", intenseScan)
addCommand("pl_dump", "Dump all the networking channels in a nice neat list.", dumpNet)
addCommand("pl_lua_run", "Sends function to selected channel.")
